...
async function test() {
    ...
    try {
        ...
        console.log(`\nSuccessfully fetched ${testTrees.length} trees for 'TEST'.`);
        if (testTrees.length > 0) {
            console.log("Sample tree matching 'TEST':", testTrees[0]);
        }

        console.log("\nFetching default trees (empty query)...");
        const defaultTrees = await controller.fetchTrees("");
        console.log(`Successfully fetched ${defaultTrees.length} default trees (limit 500).`);
